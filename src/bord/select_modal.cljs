...
(defn multiselect-modal [{:keys [state on-close options labelfn header ordered value]}]
  (if (not= @state :closed)
    (let
      ...
      [:div
       {:class (if (= @state :closing)
                 "modal modal-select modal-select-closing"
                 "modal modal-select")}
       [:div
        {:class "modal-header"}
        ...
        [:div
         ...
         [:button {:class "close" :on-click on-close} "Close"]]]
       [:div
        {:class "modal-body"}
        [:div
         {:class "available modal-section"}
         [:h3 "Available"]
         (if (some? available)
           [:div
            {:class "btn-group"}
